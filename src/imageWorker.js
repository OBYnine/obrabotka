self.onmessage = async (event) => {
    if (event.data.type === 'parseGB7') {
      try {
        ...
        // Преобразуем pixelData в обычный массив для передачи
        const pixelDataArray = Array.from(imageData.pixelData);
        
        self.postMessage({
          type: 'gb7Parsed',
          payload: {
            ...imageData,
            pixelData: pixelDataArray // Передаем как обычный массив
          }
        });
      } catch (error) {
        self.postMessage({
  ...
  function parseGB7File(buffer) {
    ...
    if (signature[0] !== 0x47 || signature[1] !== 0x42 || 
        signature[2] !== 0x37 || signature[3] !== 0x1D) {
    ...
    const width = header.getUint16(6, false);
